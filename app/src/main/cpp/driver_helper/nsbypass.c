...
bool linker_ns_load(const char* lib_search_path) {
    ...
    if(ld_android_handle == NULL) {
        LOGE("Failed to load ld-android.so");
        return false;
    }
    ...
    if(android_create_namespace == NULL || android_link_namespaces == NULL) {
        LOGE("Failed to get namespace functions from ld-android.so");
    ...
    driver_namespace = local_android_create_namespace("ralaunch-driver",
                                                      full_path,
                                                      full_path,
    ...
    // You need to link the new driver_namespace with NULL and add ld-android.so
    ...
    dlclose(ld_android_handle);
    LOGI("Driver namespace created successfully");
    return true;
#else
    return false;
#endif
...
#ifdef __ANDROID__
    if (driver_namespace == NULL) {
        LOGE("Driver namespace not initialized, call linker_ns_load() first");
        return NULL;
    }
...
void* linker_ns_dlopen_unique(const char* tmpdir, const char* name, int flags) {
    ...
    void* result = android_dlopen_ext(pathbuf, flags, &extinfo);
    patch_id++;
    return result;
#else
    return NULL;
