...
Java_com_app_ralaunch_core_common_util_NativeMethods_nativeChdir(JNIEnv *env, jclass clazz,
                                                                 jstring path) {
    if (path == nullptr) {
        return -1;
    }
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_app_ralaunch_core_common_util_NativeMethods_nativeSetupStdinPipe(JNIEnv *env,
                                                                          jclass clazz) {
    int pipefd[2]; // [0]=read, [1]=write
    if (pipe(pipefd) != 0) {
        LOGE("pipe() failed: {}", strerror(errno));
        return -1;
    }

    // 将 stdin (fd 0) 重定向到管道的读端
    if (dup2(pipefd[0], STDIN_FILENO) == -1) {
        LOGE("dup2(pipe_read, stdin) failed: {}", strerror(errno));
        close(pipefd[0]);
        close(pipefd[1]);
        return -1;
    }
    ...
    if (new_stdin != NULL && new_stdin != stdin) {
        ...
        LOGI(
                "fdopen(stdin) returned new FILE* with fd={} (stdin fd={})",
                fileno(new_stdin),
                fileno(stdin)
        );
    ...
    LOGI(
            "stdin pipe setup OK: write_fd={}, stdin(fd0) -> pipe_read, isatty={}",
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_app_ralaunch_core_common_util_NativeMethods_nativeWriteStdin(JNIEnv *env, jclass clazz,
                                                                      jstring input) {
    if (s_stdin_write_fd < 0) {
        LOGW("stdin pipe not setup, ignoring write");
        return -1;
    }
    if (input == nullptr) {
        return -1;
    ...
    LOGI(
            "stdin write: \"{}\" ({} bytes)", str, written);

    env->ReleaseStringUTFChars(input, str);

    if (written < 0) {
        LOGE("write() failed: {}", strerror(errno));
        return -1;
    }
    return (jint) written;
}
...
Java_com_app_ralaunch_core_common_util_NativeMethods_nativeCloseStdinPipe(JNIEnv *env,
                                                                          jclass clazz) {
    if (s_stdin_write_fd >= 0) {
        close(s_stdin_write_fd);
        LOGI("stdin pipe write_fd closed");
        s_stdin_write_fd = -1;
    }
}
