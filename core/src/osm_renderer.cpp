...
static bool load_osmesa_library(void) {
    ...
    g_osm.CreateContextExt = (pfn_OSMesaCreateContextExt) dlsym(g_osm.lib_handle,
                                                                "OSMesaCreateContextExt");
    g_osm.DestroyContext = (pfn_OSMesaDestroyContext) dlsym(g_osm.lib_handle,
    ...
    g_osm.GetProcAddress = (pfn_OSMesaGetProcAddress) dlsym(g_osm.lib_handle,
    ...
    if (!g_osm.CreateContextExt || !g_osm.DestroyContext ||
        ...
        LOGE("  CreateContextExt=%p DestroyContext=%p MakeCurrent=%p PixelStore=%p",
             reinterpret_cast<void *>(g_osm.CreateContextExt),
             reinterpret_cast<void *>(g_osm.DestroyContext),
             reinterpret_cast<void *>(g_osm.MakeCurrent),
             reinterpret_cast<void *>(g_osm.PixelStore));
...
bool osm_renderer_init(ANativeWindow *window) {
    ...
    if (turnip_handle) {
        ...
        LOGW("Failed to pre-load Turnip: {}", dlerror());
        ...
        if (vulkan_handle) {
            ...
            LOGW("Failed to load any Vulkan library: {}", dlerror());
        }
    }
    ...
    LOGI("Initializing OSMesa context: {}x{}", g_osm.width, g_osm.height);
    ...
    if (!g_osm.color_buffer) {
        LOGE("Failed to allocate color buffer ({} bytes)", buffer_size);
        g_osm.DestroyContext(g_osm.context);
        g_osm.context = NULL;
        return false;
    }
    memset(g_osm.color_buffer, 0, buffer_size);
    LOGI("Color buffer allocated: {} bytes", buffer_size);
    ...
    LOGI("OSMesa renderer initialized successfully ({}x{})", g_osm.width, g_osm.height);
...
void osm_swap_buffers(void) {
    ...
    if (cur_w != g_osm.width || cur_h != g_osm.height) {
        LOGI("Window resized: {}x{} -> {}x{}, recreating buffer",
