...
namespace RALauncher::SharedEnvvars {
    ...
    static std::string get_package_name() {
        ...
        LOGW("PACKAGE_NAME not set, using default: com.app.ralaunch");
        return {"com.app.ralaunch"};
    }

    static std::string get_external_storage_directory() {
        const char *external_storage_directory_cstr = getenv(
                "EXTERNAL_STORAGE_DIRECTORY"); // RaLaunchApplication.java 中设置了
        ...
        for (int i = 0; default_paths[i] != nullptr; i++) {
            ...
            if (fs::exists(default_paths[i], ec) && fs::is_directory(default_paths[i], ec)) {
                LOGW("EXTERNAL_STORAGE_DIRECTORY not set, using fallback: {}", default_paths[i]);
                return {default_paths[i]};
            }
        }

        // 如果所有备用方案都失败，返回空字符串并记录错误
        LOGE("EXTERNAL_STORAGE_DIRECTORY not set and no fallback path available");
        return {""};
    }
