...
data class ControlLayout(
     ...
     * 获取特定类型的控件
     */
    fun getControlsByType(type: ControlType): List<ControlConfig> =
        controls.filter { it.type == type }

    /**
     * 获取可见控件
     */
    val visibleControls: List<ControlConfig>
        get() = controls.filter { it.visible }
}
